...
contract("BattleLedger", function (accounts) {
  ...
  it("Transfer of Ownership", async () => {
    ...
    const a = await battleCardInstance.isOwner(accounts[1], newCardId);

    // // transfer card to account 2
    // await battleCardInstance.transferOwnership(newCardId, accounts[2], {
    //   from: accounts[1],
    // });

    // // check if owner of card is the new owner
    // await assert.strictEqual(
    //   await battleCardInstance.getOwner(newCardId),
    //   accounts[2],
    //   "New owner does not match owner of the card"
    // );
  });

  //   it("Previous Owners Recorded", async () => {
  //     // Create card from account 1
  //     await battleCardInstance.createCard({ from: accounts[1] });
  //   });
});
